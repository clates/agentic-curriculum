...
try:
    ...
    check(response.status_code == 200, f"POST /generate_weekly_plan did not return 200. Got {response.status_code}.")
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            check(len(daily_plan) == 5, f"'daily_plan' does not have 5 items. Got {len(daily_plan)}.")
            ...
            if len(daily_plan) > 0:
                ...
                check("materials_needed" in lesson_plan, "Lesson plan is missing 'materials_needed'.")
                ...
                check(standard.get("subject") == "Math", "Theme rule failed. Standard subject is not 'Math'.")
