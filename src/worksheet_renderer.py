...
def _render_venn_diagram_image(
    ...
    *,
    ...
    max_items = max(len(worksheet.left_items), len(worksheet.right_items), len(worksheet.both_items))
    if max_items > 0:
    ...
    draw.text((width - margin - _text_width(meta_font, date_text), y), date_text, font=meta_font, fill="black")
    ...
    if worksheet.left_items or worksheet.right_items or worksheet.both_items:
        ...
        draw.text((margin + col_width, y), f"{worksheet.both_label}:", font=label_font, fill="black")
        draw.text((margin + col_width * 2, y), f"{worksheet.right_label}:", font=label_font, fill="black")
        y += label_height
        ...
        for i in range(max_items):
            ...
            if i < len(worksheet.both_items):
                draw.text((margin + col_width, y), f"• {worksheet.both_items[i]}", font=body_font, fill="black")
            if i < len(worksheet.right_items):
                draw.text((margin + col_width * 2, y), f"• {worksheet.right_items[i]}", font=body_font, fill="black")
            y += body_line_height
...
def _render_feature_matrix_image(
    ...
    *,
    ...
    # Draw header border
    draw.line((table_x, table_y + row_height, table_x + content_width, table_y + row_height), fill="black", width=2)
    ...
    for item in worksheet.items:
        ...
        for prop in worksheet.properties:
            ...
            if worksheet.show_answers and item.checked_properties and prop in item.checked_properties:
                # Draw checkmark
                draw.line(
                    (checkbox_x + 3, checkbox_y + checkbox_size // 2, checkbox_x + checkbox_size // 3, checkbox_y + checkbox_size - 4),
                    fill="black",
                    width=2,
                )
                draw.line(
                    (checkbox_x + checkbox_size // 3, checkbox_y + checkbox_size - 4, checkbox_x + checkbox_size - 3, checkbox_y + 4),
                    fill="black",
                    width=2,
                )
...
def _render_odd_one_out_image(
    ...
    *,
    ...
    if instruction_lines:
        total_height += len(instruction_lines) * instruction_line_height + reasoning_line_height // 2
    ...
    for row in worksheet.rows:
        ...
        if worksheet.show_answers:
            ...
            total_height += reasoning_line_height * (worksheet.reasoning_lines + 1)  # "Why?" + lines
...
def _render_tree_map_image(
    ...
    *,
    ...
    root_box_padding = 15
    draw.rectangle(
        (root_x - root_box_padding, y, root_x + root_text_width + root_box_padding, y + root_height),
        outline="black",
        width=3,
    )
    ...
    if num_branches > 0:
        ...
        for idx, branch in enumerate(worksheet.branches):
            ...
            label_box_padding = 8
            draw.rectangle(
                (label_x - label_box_padding, branches_y, label_x + label_width + label_box_padding, branches_y + branch_height),
                outline="black",
                width=2,
            )
